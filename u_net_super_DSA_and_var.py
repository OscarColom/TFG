...
def train_model(model, train_loader, val_loader, num_epochs=15, lr=1e-3, output_dir="outputs", checkpoint_path="checkpoint.pth", resume=False, resume_checkpoint_path=None, criterion="L1Loss", optimizer="Adam"):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for target_frame, reference_frame, input_frames in train_loader:
            ...
            sr_reference_aligned = warp(img, sr_flow)

            #Alñienamos sifma
            sr_sigma_aligned =  warp(sigma, sr_flow)
            ...
            lr_sigma_aligned = sr_sigma_aligned[:, :, ::2, ::2]
            ...
            # Calculamos la loss
            loss = (( (target_frame_grayscale - lr_reference_aligned)**2 / torch.exp(lr_sigma_aligned) + lr_sigma_aligned )/2 ).mean()
        ...
        total_sigma = 0  # Acumulador para sigma
        with torch.no_grad():
            for target_frame, reference_frame, input_frames in val_loader:
                ...
                total_mse += ((target_frame_grayscale - lr_reference_aligned)**2).mean().item()  # MSE puro
                total_sigma += torch.exp(lr_sigma_aligned).mean().item()  # Media de sigma

        val_loss_epoch /= len(val_loader)
        val_losses.append(val_loss_epoch)
        avg_mse = total_mse / len(val_loader)  # MSE promedio en la época
        avg_sigma = total_sigma / len(val_loader)  # Sigma promedio en la época
...
def process_and_save_selected_stacks_by_name(dataset, model, device, output_dir, selected_filenames):
    ...
    for stack_idx in selected_indices:
        ...
        output_image = (((output_image[0].detach().cpu()) + 1) / 2) * 4751
        output_image = torch.clamp(output_image, 0, 4751)  # Forzamos para que no de errr
